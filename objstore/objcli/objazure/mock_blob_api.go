...
// GetSASToken provides a mock function with given fields: permissions, start, expiry
func (_m *mockBlobAPI) GetSASToken(permissions azblob.BlobSASPermissions, start, expiry time.Time) (azblob.SASQueryParameters, error) {
	ret := _m.Called(permissions, start, expiry)

	var r0 azblob.SASQueryParameters
	if rf, ok := ret.Get(0).(func(azblob.BlobSASPermissions, time.Time, time.Time) azblob.SASQueryParameters); ok {
		r0 = rf(permissions, start, expiry)
	} else {
		r0 = ret.Get(0).(azblob.SASQueryParameters)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(azblob.BlobSASPermissions, time.Time, time.Time) error); ok {
		r1 = rf(permissions, start, expiry)
	} else {
		r1 = ret.Error(1)
	}
...
type mockConstructorTestingTnewMockBlobAPI interface {
	mock.TestingT
	Cleanup(func())
}
...
func newMockBlobAPI(t mockConstructorTestingTnewMockBlobAPI) *mockBlobAPI {
	mock := &mockBlobAPI{}
	mock.Mock.Test(t)
